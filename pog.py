...
def save_views_to_excel():
    try:
        ...
        excel_file = os.path.join(desktop, 'output1.xlsx')
...
def main(page: ft.Page):
    ...
    page.title = "Рекомендационная система"
    page.window_width = 600
    page.window_height = 400

    def process_excel_file(e):
        try:
            file_path = file_picker.result.files[0].path if file_picker.result else None
            if file_path:
                ...
                #delete()
                res = ft.AlertDialog(title=ft.Text(f"Итоговый файл сохранён на рабочий стол"),
                                     on_dismiss=lambda e: print('Error'))
                page.dialog = res
                res.open = True
                page.update()
        except Exception as e:
            res = ft.AlertDialog(title=ft.Text(f"Ошибка, неверный файл"), on_dismiss=lambda e: print('Erorr'))
            page.dialog = res
            res.open = True
            page.update()
            print(e)
    ...
    pick_files_button = ft.ElevatedButton("Выбрать файл Excel", on_click=pick_files)

    page.add(pick_files_button)
