...
def insert_to_family(file_path):
    ...
    try:
        for index, row in df.iterrows():
            ...
            """, (int(index), row['Округ'], row['район'], int(row['№ кв-ры']), row['Вид засел.'],
...
def match_new_apart_to_family():
    try:
        ...
        df_new_apart = pd.DataFrame(new_aparts,
                                    columns=['new_apart_id', 'district', 'area', 'room_count',
                                             'total_living_area',
                                             'living_area', 'floor'])
        ...
        for _, old_apart in df_old_apart.iterrows():
            ...
            if not suitable_aparts.empty:
                ...
                cursor.execute(
                    f"INSERT INTO recomendation.offer (family_id, new_apart_id) VALUES ({family_id}, {new_apart_id})")
                conn.commit()
...
def save_views_to_excel():
    try:
        ...
        views = ['available_apart', 'svod_balance', 'result_of_recomendation', 'balance', 'ranked',
                 'cannot_to_excel']
